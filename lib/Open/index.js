...
module.exports = {
  ...
  s3: function (client, params, options) {
    var source = {
      size: function () {
        const command = new S3.HeadObjectCommand(params);
        return client.send(command).then(function (res) {
          return res.ContentLength;
        });
      },
      stream: function (offset, length) {
        const command = new S3.GetObjectCommand({ ...params, Range: 'bytes=' + offset + '-' + (length ? length : '') });
        return client.send(command).then(function (res) {
          return res.Body;
        })
      }
